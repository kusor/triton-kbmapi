...
// XXX: Remove, this is just for initial testing purposes
function testAuth(req, res, next) {
    var fs = require('fs');
    var mod_path = require('path');
    var path = mod_path.resolve(__dirname, '..',
        'test/one_token_test_edcsa.pub');
    var pubkey = fs.readFileSync(path, 'ascii');
    req.token = {
        pubkeys: {
            '9e': pubkey
        }
...
function register(http, before) {
    http.get({
        path: '/auth',
        name: 'auth'
    }, before, testAuth, mod_auth.signatureAuth, function (req, res, next) {
        res.send(202);
        next();
    });
    http.get({
    ...
    http.del({
        path: '/pivtokens/:guid',
        name: 'deltoken'
    }, before, preloadToken, deleteToken);
    http.get({
        path: '/pivtokens/:guid/pin',
        name: 'gettokenpin'
    }, before, preloadToken, getTokenPin);
}
